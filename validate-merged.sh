...
run_base() {
    ...
    bench_flash_attn() {
        ...
        TRAINING_CMD="$TRAINING_BASE_CMD \
                              --loss-file $MERGER_DIR/results/$DTYPE/$SRUN_OUTFILE_SUFFIX-$out_file_prefix-$SEQ_LEN-1.csv \
                              $TRAINING_ARGS"
        srun_outfile=$(get_srun_outfile "$MERGER_DIR/logs" "$SRUN_OUTFILE_SUFFIX-$out_file_prefix" "$SEQ_LEN-1")
        ...
        TRAINING_CMD="$TRAINING_BASE_CMD \
                              --loss-file $MERGER_DIR/results/$DTYPE/$SRUN_OUTFILE_SUFFIX-$out_file_prefix-$SEQ_LEN-2.csv \
                              $TRAINING_ARGS"
        srun_outfile=$(get_srun_outfile "$MERGER_DIR/logs" "$SRUN_OUTFILE_SUFFIX-$out_file_prefix" "$SEQ_LEN-2")
        echo "Writing to: $srun_outfile"
        srun --output $srun_outfile \
             --cpus-per-task $SLURM_CPUS_PER_TASK \
             bash -c "$TRAINING_CMD"

        echo "--------------- FAv3: $flash_attn   No Checkpt   Seq Len: $SEQ_LEN   Complete! ----------------"
    }
...
run_with_break() {
    ...
    bench_flash_attn() {
        ...
        TRAINING_CMD="$TRAINING_BASE_CMD \
                              --loss-file $MERGER_DIR/results/$DTYPE/$SRUN_OUTFILE_SUFFIX-$out_file_prefix-$SEQ_LEN-part1.csv \
                              $TRAINING_ARGS"
        ...
        TRAINING_CMD="$TRAINING_BASE_CMD \
                              --loss-file $MERGER_DIR/results/$DTYPE/$SRUN_OUTFILE_SUFFIX-$out_file_prefix-$SEQ_LEN-part2.csv \
                              --load-checkpoint \
                              $TRAINING_ARGS"
        srun --output $srun_outfile \
             --cpus-per-task $SLURM_CPUS_PER_TASK \
...
# # # #  run_base "base" "$SEQ_LEN" "$TRAIN_ARGS" [ "flash" | "torch" ]
run_base "base" "$SEQ_LEN" "$TRAIN_ARGS"
run_base "pccheck" "$SEQ_LEN" "$TRAIN_ARGS" "flash"

# # # #  run_with_break "Checkpoint mode" "$SEQ_LEN" "$TRAIN_ARGS" [ "flash" | "torch" ] [ STOP_AT_STEP ]
run_with_break "pccheck" "$SEQ_LEN" "$TRAIN_ARGS" "flash"

cd $MERGER_DIR/results && python plot_loss.py
